...
def test_backend_docs_keep_mode_and_review_smoke() -> None:
    ...
    _assert_mentions(
        ...
        "subordinate `superpowers` skills",
...
def test_codex_install_docs_cover_discovery_install_and_verification() -> None:
    ...
    _assert_mentions(
        ...
        "source namespace is `neurapaw-delivery`",
    ...
    assert "delivery-flow:delivery-flow" not in _normalized(install_doc)
...
def test_platform_docs_cover_bootstrap_install_paths() -> None:
    ...
    assert "delivery-flow:delivery-flow" not in _normalized(readme_codex)
    assert "~/.agents/skills/delivery-flow/using-delivery-flow/skill.md" not in _normalized(
        codex_install
    )
    assert "~/.agents/skills/delivery-flow/implementation-review/skill.md" not in _normalized(
...
def test_project_readmes_cover_current_machine_install_and_verification() -> None:
    ...
    _assert_mentions(
        ...
        "必须先选择",
        "开始改代码",
        "delivery-flow` 的 execution_strategy 选项",
        "delivery-flow` 都仍是顶层 controller",
        "loaded neurapaw-delivery:delivery-flow as top-level controller",
        "不会再次打开通用执行方式选择",
...
def test_verification_scenarios_cover_execution_strategy_edges() -> None:
    ...
    _assert_mentions(
        ...
        "implementation review handoff starts fix run",
        "explicit namespaced activation is observable",
        "loaded neurapaw-delivery:delivery-flow as top-level controller",
        "superpowers:*",
        "activation as failed",
        "start a fix run from the implementation-review findings",
        ...
        "delivery-flow` remains the top-level controller",
        "which approach?",
        "owner selects `execution_strategy`",
