...
Codex 会用 `.codex-plugin/plugin.json` 里的 `neurapaw-delivery` 作为来源
namespace 展示这些入口，例如 `neurapaw-delivery:delivery-flow`。
...
- [skills/delivery-flow/SKILL.md](./skills/delivery-flow/SKILL.md)
  面向 AI 的执行 skill 合约，也包含进入 `delivery-flow` 之后的路由与选择规则。
...
- [skills/implementation-review/SKILL.md](./skills/implementation-review/SKILL.md)
  面向 AI 的通用 implementation review skill，用于检查 spec compliance、架构适配、测试质量和交付就绪度。
...
- `skills/implementation-review/`
  项目无关的通用 implementation review skill。
- `tests/`
  仓库测试基线。
...
| 已经有 plan，不需要持续交付 ownership，且工作可以稳定线性执行后结束 | `executing-plans` |
